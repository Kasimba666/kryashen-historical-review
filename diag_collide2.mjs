...
function extractCsrf(text) {
  const m = text.match(/name="csrf-token"\s+content="([^"]+)"/) || text.match(/name="csrfToken"\s+(?:value|content)="([^"]+)"/) || text.match(/csrfToken[=:]\s*"?([a-zA-Z0-9]{16,})/i) || text.match(/csrf-token[=:]\s*"?([a-zA-Z0-9]{16,})/i)
  return m ? m[1] : null
}
async function getComponentToken() {
  let r = await fetch(BASE + '/ru/login', { headers: cookieHeader() ? { Cookie: cookieHeader() } : {}, redirect:'manual' })
  let tok
  if (r.status === 302) { const dash = await (await fetch(r.headers.get('location'), { headers:{ Cookie: cookieHeader() } })).text(); tok = extractCsrf(dash) }
  else tok = extractCsrf(await r.text())
  if (!tok) { r = await fetch(BASE + '/ru', { headers:{ Cookie: cookieHeader() }, redirect:'manual' }); const t2 = r.status===302 ? await (await fetch(r.headers.get('location'), { headers:{ Cookie: cookieHeader() } })).text() : await r.text(); tok = extractCsrf(t2) }
  return tok
}
...
async function createIssue(n, fixedPath) {
  const tok = await getComponentToken()
  const before = await count()
  const form = new URLSearchParams()
  form.set('volume', '5'); form.set('number', '5'); form.set('year', '2026')
  form.set('showVolume','1'); form.set('showNumber','1'); form.set('showYear','1'); form.set('showTitle','1')
  form.set('urlPath', fixedPath)
  form.set('submitFormButton','1')
  form.set('title[ru_RU]', 'Повтор'); form.set('description[ru_RU]','t')
  form.set('title[ru]', 'Повтор'); form.set('title[en]', 'Повтор')
  ...
  console.log('[create ' + n + ' path=' + fixedPath + '] before=' + before + ' after=' + after + ' ' + (after>before?'OK':'FAIL') + ' respLen=' + ct.length)
  console.log('  formHasErrors?', /class="[^"]*formError[^"]*"/.test(ct))
  const nt = ct.match(/id="issueDataNotification"[^>]*>([\s\S]*?)<\/div>/i)
  if (nt) { const t = nt[1].replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim(); if (t) console.log('  notification:', t) }
  const fs = await import('fs'); fs.writeFileSync('resp'+n+'.html', ct)
  return after > before
}
// both use SAME fixed urlPath -> should collide on 2nd
const ok1 = await createIssue(1, 'samefixedpath')
const ok2 = await createIssue(2, 'samefixedpath')
console.log('results:', ok1, ok2)
const issues = (await (await fetch(BASE + '/api/v1/issues', { headers: restHeaders() })).json()).items || []
const tok = await getComponentToken()
for (const d of issues.filter(it => /Повтор/.test((it.title&&(it.title.ru||it.title.en||''))||''))) {
