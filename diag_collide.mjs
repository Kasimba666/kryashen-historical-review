...
function extractCsrf(text) {
  const m = text.match(/name="csrf-token"\s+content="([^"]+)"/) ||
            ...
            text.match(/csrfToken[=:]\s*"?([a-zA-Z0-9]{16,})/i) || text.match(/csrf-token[=:]\s*"?([a-zA-Z0-9]{16,})/i)
  return m ? m[1] : null
}
...
// create with IDENTICAL metadata (simulating user creating 2 issues with same defaults)
async function createIssue(n) {
  const tok = await getComponentToken()
  const before = await count()
  const slug = 'collide-' + Date.now()
  const form = new URLSearchParams()
  form.set('volume', '1'); form.set('number', '1'); form.set('year', '2026')
  form.set('showVolume','1'); form.set('showNumber','1'); form.set('showYear','1'); form.set('showTitle','1')
  form.set('urlPath', slug)   // explicit same-ish path each time (collision test)
  form.set('submitFormButton','1')
  form.set('title[ru_RU]', 'ОдинИтотЖеТест'); form.set('description[ru_RU]','t')
  form.set('title[ru]', 'ОдинИтотЖеТест'); form.set('title[en]', 'ОдинИтотЖеТест')
  ...
  console.log('  formHasErrors(.formError)?', /class="[^"]*formError[^"]*"/.test(ct))
...
const tok = await getComponentToken()
for (const d of issues.filter(it => /ОдинИтотЖеТест/.test((it.title&&(it.title.ru||it.title.en||''))||''))) {
